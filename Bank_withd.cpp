...
class CurrentAccount : public BankAccount {    // create current account
private:
    double interestRate;

public:
    CurrentAccount(int accNo, double bal, double rate)
        : BankAccount(accNo, bal) {
        interestRate = rate;
    }
...
int main() {
    ...
    CurrentAccount::Loan loan1(50000);
    loan1.showLoan();

    CurrentAccount cacc(303, 10000, 10);
    cacc.showBalance();
    cacc.addInterest();

    // Using nested class inside derived class
    CurrentAccount::Loan loan2(50000);
    loan2.showLoan();
